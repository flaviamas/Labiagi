...
void OrazioWrapper::connect(const char* serial_device) {
  ...
  _client = OrazioClient_init(serial_device,...);
  //
  if (! _client) {
  ...
  for (int i=0; i<retries; ++i){
	...
	PacketStatus pk = OrazioClient_sync(orazio_client,retries);
  ...
  //        in the negative case disconnect orazio
	...
	if(!pk){
		 printf("Errore nella sincronizzazione \n");
		 fflush(stdout);
		 disconnect();
	 }
  ...
  //        in the negative case disconnect orazio
	pk =  OrazioClient_readConfiguration(orazio_client,retries);
	if(!pk){
		 printf("Errore nella sincronizzazione \n");
		 fflush(stdout);
		 disconnect();
  ...
  //        in the negative case disconnect orazio
	...
	pk = OrazioClient_get(_client, (PacketHeader*)& system_params);		
	if(!pk){
		 printf("Errore nella sincronizzazione \n");
		 fflush(stdout);
		 disconnect();
  ...
  //        in the negative case disconnect orazio
	...
	pk = OrazioClient_sendPacket(_client, (PacketHeader*)& system_params, retries);
	if(!pk){
		 printf("Errore nella sincronizzazione \n");
		 fflush(stdout);
		 disconnect();
...
void OrazioWrapper::getOdometry(float& x, float& y, float& theta) {
  // TODO : get the odometry measure from drive status
	OrazioClient_getOdometryPosition(_client, x,y,z);
  //
}
...
void OrazioWrapper::setVelocities(float tv, float rv) {
  // TODO : Set velocities of the platform using drive control
	...
	OrazioClient_setBasesVelocity(_client,tv,rv);
